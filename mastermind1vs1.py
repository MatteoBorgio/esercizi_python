...
def scegli_sequenza(dimensione: int = 4, minimo: int = 1, massimo: int = 6):
    ...
    while len(sequenza_segreta) < dimensione:
            try:
                numero = int(g.getpass(f"Inserisci il {count+1} numero: "))
...
def calcola_feedback(sequenza_segreta: list, sequenza_utente: list):
    ...
    return len(elementi_posto_giusto), len(elementi_posto_sbagliato)
...
def gioca_partita(tentativi: int = 10, scelta: int = 1):
    if scelta == 2:
    ...
    for i in range(tentativi):
        ...
        print("")
        print(f"Hai inserito {feedback[0]} elementi al posto giusto")
        print(f"Hai inserito {feedback[1]} elementi giusti ma al posto sbagliato")
        print("")
...
def main():
    ...
    # Avvia la partita
    gioca_partita(tentativi, scelta)
